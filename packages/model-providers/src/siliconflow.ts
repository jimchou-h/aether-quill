 ...
 */
interface SiliconFlowChatCompletionResponse {
  id: string;
  object: string;
  created: number;
  model: string;
...
export class SiliconFlowProvider implements ModelProvider {
  ...
  constructor(config: SiliconFlowConfig) {
    this.defaultModel = config.defaultModel || 'Qwen/Qwen2-7B-Instruct';
 ...
 * SiliconFlow 供应商工厂
 */
export class SiliconFlowFactory {
  create(config: SiliconFlowConfig): SiliconFlowProvider {
