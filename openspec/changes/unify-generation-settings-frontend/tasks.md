...
- [x] 1.3 AI 调用错误透传：缺 key 时 `LlmProviderKeyMissing`（或等价）可读文案；上游 4xx 含 model + 原文摘要；API/SSE 到前端可展示
...
- [x] 2.3 AI/SSE 错误展示使用服务端可读 `message`（不再只显示码 400）
...
- [x] 4.1 相关 typecheck/test；手测或单测：缺 key / 错误 model 时前端能看到原因文案
