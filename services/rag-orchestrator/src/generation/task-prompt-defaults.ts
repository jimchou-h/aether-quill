...
export const WAREHOUSE_TASK_PROMPT_DEFAULTS: Record<string, string> = {
  ...
  'chapter.compliance.outline': [
    ...
    '若提供【人物卡参考】：须核对正文与人设、称谓、关系是否一致；不一致项写入大纲（category 可用 consistency）。',
  ...
  'chapter.pipeline.sensory.rewrite': [
    ...
    '【维度边界】本步骤 ONLY 负责本模块职责；禁止修改其他维度的内容。',
    'ONLY：按 <writing-brief> 提升亲密场景的感官描写质量。',
  ...
  'chapter.pipeline.character.coverage.verify': [
    ...
    '【维度边界】本步骤 ONLY 负责本模块职责；禁止修改或输出正文。',
    'ONLY：逐条判断大纲编辑意图是否在正文中自然落实；禁止修改或输出正文。',
    '只输出 JSON：{"items":[{"id":"r1","status":"done|partial|missed","note":"简要说明"}]}',
    'done=读者能自然感知且无突兀插入；partial=有体现但融合度不足；missed=基本未体现。',
    'note 须说明自然程度或生硬原因；必须覆盖输入大纲中的每一条（按 id 对应），不得遗漏。',
  ].join('\n'),
  ...
  'chapter.pipeline.character.rewrite.fix-items': [
    ...
    '【维度边界】本步骤 ONLY 负责本模块职责；禁止越界修改未列出的维度。',
    'ONLY：围绕 <writing-brief> 小范围重织相关段落与过渡句，使补修自然融入原文。',
    '可调整相邻过渡句与句序；保持剧情事实、对白含义、人物关系不变。',
    '禁止为完成条目简单插句；直接输出完整正文，不要说明或 Markdown。',
  ].join('\n'),
  ...
  'chapter.compliance.coverage.verify': [
    '你是一位资深小说编辑，正在对照已确认合规大纲验收章节改写稿的落实情况。',
    'ONLY：逐条判断合规编辑意图是否在正文中自然落实；禁止修改或输出正文。',
    '只输出 JSON：{"items":[{"id":"r1","status":"done|partial|missed","note":"简要说明"}]}',
    'done=读者能自然感知且无突兀插入；partial=有体现但融合度不足；missed=基本未体现。',
    '必须覆盖输入大纲中的每一条（按 id 对应），不得遗漏。',
  ].join('\n'),
  'chapter.compliance.rewrite.fix-items': [
    '你是一位资深内容合规编辑，正在按合规编辑意图子集对章节正文做织入式补修。',
    'ONLY：围绕 <writing-brief> 落实合规修改项；小范围重织相关段落，保持叙事风格。',
    '禁止为完成条目硬插说明句；直接输出完整正文，不要 JSON、不要说明或 Markdown。',
  ].join('\n'),
